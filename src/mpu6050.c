...
double No_Kalman_Fliter(MPU6050_t *DataStruct){
	  uint8_t Rec_Data[14];
	    int16_t temp;
...
//	    double dt = (double) (HAL_GetTick() - timer) / 1000;
	    ...
	    double roll_sqrt = sqrt(
	            DataStruct->Accel_X_RAW * DataStruct->Accel_X_RAW + DataStruct->Accel_Z_RAW * DataStruct->Accel_Z_RAW);
	    if (roll_sqrt != 0.0) {
	        DataStruct->roll = atan(DataStruct->Accel_Y_RAW / roll_sqrt) * RAD_TO_DEG;
	    } else {
	        DataStruct->roll = 0.0;
	    }
	    DataStruct->pitch = atan2(-DataStruct->Accel_X_RAW, DataStruct->Accel_Z_RAW) * RAD_TO_DEG;
